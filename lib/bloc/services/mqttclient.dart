...
/// to the MQTT specification. The example is runnable, also refer to devicemqtt_client_broker_test...dart
...
Future<int> main() async {
  ...
  final MqttClient client = MqttClient('www.studify.in', '');
  ...
  try {
    ...
    await client.connect('iotic','iotic');
  ...
  const String topic =
      'device/#'; // Wildcard topic, you wont see these, see below
  client.subscribe(topic, MqttQos.atMostOnce);
  const String topic1 = 'device/#'; // Wildcard topic
  client.subscribe(topic1, MqttQos.atMostOnce);
  print(
      'EXAMPLE::We have subscribed to device# and device# but due to topic filtering you will only see messages for device#');
  ...
  final MqttClientTopicFilter topicFilter =
      MqttClientTopicFilter('device/+', client.updates);
  ...
  topicFilter.updates.listen((List<MqttReceivedMessage<MqttMessage>> c) {
    ...
    print(
        'EXAMPLE::Filtered Change notification for device#:: topic is <${c[0].topic}>, payload is <-- $pt -->');
    print('');
  });
